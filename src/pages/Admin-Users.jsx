...
import { Link, Navigate } from 'react-router-dom';
 ...
 const AdminUsers = () => {
  ...
  const getAllUsersData = async() => {
    try{
      const response = await fetch("https://backend-pam6.vercel.app/api/admin/users", {
        method : "GET",
        headers : {
  ...
  const deleteUser = async(id) => {
    try{
      const response = await fetch(`https://backend-pam6.vercel.app/api/admin/users/delete/${id}`, {
        method : "DELETE",
        headers : {
          Authorization : authorizationToken,
        }
